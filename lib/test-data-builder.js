 ...
 */
TestDataBuilder.ref = function(path, template) {
...
TestDataBuilder.prototype._resolveValues = function(values) {
  ...
  for (var key in values) {
    ...
    if (val instanceof Reference) {
      ...
      val = values[key].map(
        function (obj) {
          return obj.resolveFromContext ? obj.resolveFromContext(this._context) : obj
        },
        this
      )
    } else if (isObject(val)) {
      val = this._resolveValues(val)
    }
    result[key] = val;
  }
  return result;
 ...
 * @private
 */
function Reference(path, template) {
  this._path = path;
...
Reference.prototype.resolveFromContext = function(context) {
  ...
  return this._template ? format(this._template, result) : result;
};
